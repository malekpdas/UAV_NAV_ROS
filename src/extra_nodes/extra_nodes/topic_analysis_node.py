...
class OdomGpsComparator(Node):
    def __init__(self):
        ...
        self.declare_parameter('sub_topics.odometry.value', '/ekf/odom')
        self.declare_parameter('sub_topics.filtered_imu.value', '/ekf/filtered_imu')

        self.gps_origin = None

        self.create_subscription(NavSatFix, self.get_parameter('sub_topics.gps_fix.value').value, self.cb_gps_fix, 10)
        self.create_subscription(TwistWithCovarianceStamped, self.get_parameter('sub_topics.gps_vel.value').value, self.cb_gps_vel, 10)
        self.create_subscription(Odometry, self.get_parameter('sub_topics.odometry.value').value, self.cb_odom, 10)
        self.create_subscription(Imu, self.get_parameter('sub_topics.filtered_imu.value').value, self.cb_imu, 50)
