...
class MagCalNode(Node):
    ...
    def cb(self, msg):
        ...
        if self.counter % PLOT_EVERY_N == 0:
            self.update()
    ...
    def update(self):
        if len(self.samples) < MIN_SAMPLES:
            ...
            for ax in self.axes:
                ax.cla()
                ax.text(0.5, 0.5, f"Collecting data: {len(self.samples)}/{MIN_SAMPLES}", 
        ...
        gap_pct = compute_gap_percent(M, ANGULAR_BINS)
        ...
        self.fig.suptitle(
            f"N={len(M)} | Gap={gap_pct:.2f}% (Target < {GAP_THRESHOLD_PERCENT}%) | Wobble={wobble:.3e}"
        )

        plt.pause(0.001)

        # Stop criterion
        if gap_pct < GAP_THRESHOLD_PERCENT:
            self.finish()
